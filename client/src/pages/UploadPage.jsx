...
function UploadPage({ folder }) {
  ...
  useEffect(() => {
  fetch(`https://photo-gallery-backend-l605.onrender.com/api/images/${folder.id}`)
    .then(res => res.json())
...
const handleDelete = async (id) => {
  try {
    await fetch(`https://photo-gallery-backend-l605.onrender.com/api/images/${id}`, {
      method: "DELETE",
    });
...
const handleReplaceClick = (id) => {
  ...
  try {
    ...
    const res = await fetch("https://photo-gallery-backend-l605.onrender.com/api/images", {
      method: "POST",
      body: formData,
    });
    ...
    await fetch(`https://photo-gallery-backend-l605.onrender.com/api/images/${replaceId}`, {
      method: "DELETE",
    });
    ...
    <div className="grid">
      ...
      {images.map((img, index) => (
        <div className="image-card" key={img.id} onClick={() => { if (editingId === img.id) return; setActiveIndex(index);}}>
          <img src={`https://photo-gallery-backend-l605.onrender.com${img.filepath}`} alt="images" />
    ...
    {preview && (
      ...
      {activeIndex !== null && (
         <div className="viewer-overlay">
            ...
            <img
               src={`https://photo-gallery-backend-l605.onrender.com${images[activeIndex].filepath}`}
